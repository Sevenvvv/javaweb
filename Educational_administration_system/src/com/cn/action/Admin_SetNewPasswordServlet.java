...
public class Admin_SetNewPasswordServlet extends HttpServlet {
	...
	public void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		TblAdmin oldtbladmin = ((TblAdmin) session.getAttribute("auser"));
		TblAdmin newtbladmin = new TblAdmin();
		newtbladmin.setAdminID(oldtbladmin.getAdminID());
		newtbladmin.setAdminName(oldtbladmin.getAdminName());
		
		boolean i = false;
		String op = tblNewPswBiz.checkpswAdmin(oldtbladmin).getOldPassword();
		...
		if (oldPassword == null || "".equals(oldPassword)){
			...
			if(oldPassword.equals(op)){
				i = true;
				newtbladmin.setOldPassword(oldPassword);
			}else{
		...
		if (newPassword == null || "".equals(newPassword)){
			...
			newtbladmin.setaPassword(newPassword);
		}
		...
		if("".equals(errMsg)){
			if(i){
				boolean flag = tblNewPswBiz.newpswAdmin(newtbladmin);
				if(flag){
					request.setAttribute("errMsg", "修改密码成功请重新登录");
					request.getRequestDispatcher("login.jsp").forward(request, response);
				}else{
					request.setAttribute("errMsg", "修改失败");
					request.getRequestDispatcher("admin_SetNewPassword.jsp").forward(request, response);
				}
			}
		}else{
