					...
					<div class="page">
						<div class="pul">
							<ul class="pagination">
								...
								<c:if test="${pageBean.pageNum > 1 }">
									<li><a href="student_Courselist?pageNum=${pageBean.pageNum - 1 }">上一页</a></li>
								</c:if>
								<c:forEach begin="1" end="${pageBean.pageCount }" step="1" var="i">
					                <c:if test="${page.pageNum==i }">
					                    <li><a href="student_Courselist?pageNum=${i}">${i}</a></li>
					                </c:if>
					                <c:if test="${page.pageNum!=i }">
					                    <li><a href="student_Courselist?pageNum=${i}">${i}</a></li>
					                </c:if>
								...
								<c:if test="${pageBean.pageNum < pageBean.pageCount }">
									<li><a href="student_Courselist?pageNum=${pageBean.pageNum + 1 }">下一页</a></li>
								</c:if>
							</ul>
						</div>
					</div>
